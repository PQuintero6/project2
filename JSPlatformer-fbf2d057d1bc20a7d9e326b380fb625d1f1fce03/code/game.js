...
var actorChars = {
  "@": Player,
  "o": Pumpkin, // A pumkin will wobble up and down
  "=": Skull, "|": Skull, "v": Skull
};
...
// skull is initialized based on the character, but otherwise has a
// size and position
function Skull(pos, ch) {
  this.pos = pos;
  ...
  if (ch == "=") {
    ...
    // Drip skull. Repeat back to this pos.
    this.speed = new Vector(0, 3);
    this.repeatPos = pos;
  }
}
Skull.prototype.type = "skull";
...
Skull.prototype.act = function(step, level) {
...
Player.prototype.moveX = function(step, level, keys) {
  ...
  // Handle skull by calling playerTouched
  if (obstacle)
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  // if the player touches skull and the player hasn't won
  // Player loses
  if (type == "skull" && this.status == null) {
    this.status = "lost";
    this.finishDelay = 1;
    ...
    // If there aren't any pumpkin left, player wins
    if (!this.actors.some(function(actor) {
           return actor.type == "pumpkin";
         })) {
      this.status = "won";
      this.finishDelay = 1;
    }
